...
<div class="relative w-full" bind:this={containerRef}>
  ...
  >
    <div class="flex flex-wrap items-center gap-1.5 overflow-hidden flex-1 py-0.5">
      ...
      {:else}
        {#each selected as item}
          ...
          >
            <span
              class="truncate max-w-[150px] {item === '直连'
                ? 'text-emerald-400 font-semibold'
                : item === 'block'
                  ? 'text-rose-400 font-semibold'
                  : 'text-cyan-300'}">{item}</span
            >
            <button
              type="button"
    ...
    <div class="flex items-center gap-1.5 shrink-0 ml-1 mt-1 text-slate-500">
      {#if selected && selected.length > 0}
        <span
          class="text-[10px] font-mono text-slate-400 bg-slate-900 px-1.5 py-0.5 rounded border border-slate-800"
          >共 {selected.length} 项</span
        >
      {/if}
      <ChevronDown
        size={13}
        class="transition-transform duration-150 {isOpen ? 'rotate-180 text-cyan-400' : ''}"
      />
    </div>
  </div>
  ...
  {#if isOpen}
    <div
      tabindex="-1"
      class="absolute left-0 z-50 w-full min-w-[340px] max-w-lg bg-slate-900 border border-slate-700/90 rounded-lg shadow-2xl p-2.5 space-y-2 {openUpward
        ? 'bottom-full mb-1.5'
        : 'top-full mt-1.5'}"
    >
      <!-- Search Input -->
      ...
      <div
        class="flex items-center justify-between text-[11px] text-slate-400 px-1 border-b border-slate-800 pb-1.5"
      >
      ...
      <!-- Options List -->
      <div
        class="max-h-[420px] overflow-y-auto space-y-0.5 pr-1 [scrollbar-width:thin] [scrollbar-color:#475569_transparent]"
      >
        {#each filteredOptions as opt}
          ...
          <div
            ...
            class="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-slate-800/80 cursor-pointer text-xs transition-colors select-none {isChecked
          ...
          >
            ...
            />
            <span
              class="font-mono flex-1 truncate text-xs {isChecked
            ...
            {#if opt.type === 'direct'}
              <span
                class="text-[10px] px-1 py-0.2 rounded font-mono bg-emerald-950/80 border border-emerald-800/60 text-emerald-400"
                >直连</span
              >
            {:else if opt.type === 'block'}
              <span
                class="text-[10px] px-1 py-0.2 rounded font-mono bg-rose-950/80 border border-rose-800/60 text-rose-400"
                >阻断</span
              >
            {:else}
              <span
                class="text-[10px] px-1 py-0.2 rounded font-mono bg-cyan-950/80 border border-cyan-800/60 text-cyan-400"
                >分组</span
              >
            {/if}
          </div>
        ...
        {#if filteredOptions.length === 0}
          <div class="py-4 text-center text-slate-500 text-xs font-mono">无匹配节点分组</div>
        {/if}
      </div>
    </div>
  {/if}
