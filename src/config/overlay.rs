...
pub fn apply_runtime_overlay(mut config: Value, network: &NetworkSettings) -> Result<Value> {
    ...
    network.validate()?;
    apply_tproxy_overlay(&mut config, network)?;
    Ok(config)
}
...
fn is_cnip_rule(rule: &Value) -> bool {
    match rule.get("rule_set") {
        Some(Value::String(tag)) => tag == "cnip",
        Some(Value::Array(tags)) => tags.len() == 1 && tags[0].as_str() == Some("cnip"),
        _ => false,
    }
}
...
mod tests {
    ...
    fn test_replaces_tun_inbound_with_tproxy_set() {
        ...
        assert!(inbounds.iter().all(|inbound| inbound["tag"] != "tun-in"));
        assert_eq!(config["route"]["default_mark"], 0x67890);
        assert_eq!(config["route"]["auto_detect_interface"], false);
    }

    #[test]
    fn test_bypass_cn_does_not_require_or_rewrite_source_cnip() {
        let with_cnip = overlay(cnip_source());
        let rules = with_cnip["route"]["rules"].as_array().unwrap();
        assert!(rules.iter().any(is_cnip_rule));
    ...
    fn test_load_and_write_same_path_does_not_clobber_source_before_read() {
        ...
        assert_eq!(roundtrip["inbounds"][1]["tag"], TPROXY_IN);
